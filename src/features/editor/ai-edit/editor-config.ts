...
3) For EACH of the N shots output a generate op with a full prompt built as "<the fixed character description>, <this shot's pose/action/emotion>, <setting>, cinematic film still, SEMI-photorealistic (stylised realism — NOT a flat photo, NOT cartoon/comic-ink), realistic skin, dramatic moody lighting, shallow depth of field". Repeat the EXACT character description in EVERY shot (same person throughout). Keep the SEMI-photoreal look on every shot.
   ...
   LIP-SYNC / TALKING shots: if the user wants the character to TALK / lip-sync / speak on camera (e.g. "2 shots lip-synced", "she talks", "he says something"), make those shots VIDEO and put the spoken words in the prompt right after the word says, in quotes — e.g. { "op":"generate", "kind":"video", "prompt":"the woman at the rain-soaked window, moody cinematic, says 'ok, I will go there'", "duration":4, "aspect_ratio":"…" }. The video model reads the says '…' and animates the mouth (lip-sync) — there is NO separate lip-sync model. Keep the spoken line SHORT (a few seconds of speech), in the character's voice, fitting the story beat. Make ONLY as many talking shots as the user asked — the rest stay images / normal video.
...
2) Output N shot ops, one per narration beat, each RELEVANT to what that line says. Each shot is EITHER AI-generated OR real stock footage — pick per beat:
   ...
   LIP-SYNC / TALKING shots: if the user wants a person to TALK / lip-sync / speak on camera, make those shots VIDEO with the spoken words in the prompt right after the word says, in quotes — e.g. { "op":"generate", "kind":"video", "prompt":"a news anchor at the desk, studio lighting, says 'breaking news tonight'", "duration":4, "aspect_ratio":"…" }. The video model reads the says '…' and animates the mouth (lip-sync) — NO separate model. Keep the spoken line SHORT; make only as many talking shots as the user asked.
