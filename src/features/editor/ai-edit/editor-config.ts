...
2) Output N shot ops, one per narration beat, each RELEVANT to what that line says. Each shot is EITHER AI-generated OR real stock footage — pick per beat:
   ...
   • AI video: { "op":"generate", "kind":"video", "prompt":"…the MOTION + natural AMBIENT SOUND cues (traffic, wind, crowd, water…), as NATURAL SENTENCES (not a comma keyword list — tag-piles hurt the video model)…", "duration":<their seconds>, "aspect_ratio":"<the chosen ratio>" } — ALWAYS put ambient-sound cues in a VIDEO prompt
   • STOCK footage (real-world b-roll — cities, nature, crowds, objects, places): { "op":"search", "kind":"image|video", "query":"3-5 keyword search query", "aspect_ratio":"<ratio>", "source":"pexels", "count":1 } (ALWAYS include aspect_ratio). SOURCE: default "pexels" (modern b-roll); use "archive" for Internet Archive (historical / vintage / archival / higher-quality footage — use it when the user asks for archival/old/historical/documentary footage or better quality), or "openverse"/"wikimedia" for free/CC. Set source to whatever the user asks. — cheaper + real; PREFER stock for generic real-world beats, AI-generate for anything specific/stylised the search won't have. If the user asks for a SPECIFIC NUMBER of stock shots (e.g. "2 stock images, 1 stock video"), you MUST emit EXACTLY that many search ops of that kind (a separate op each) — do NOT AI-generate those beats.
...
- "NARRATOR: …" line → a NON-talking shot that SHOWS what the narration is about (the character(s) or the scene). Pick the best medium for the beat:
    ...
    • b-roll video (for a moving beat): { "op":"generate", "kind":"video", "prompt":"<the scene + the MOTION + natural AMBIENT SOUND cues (wind, rain, city, footsteps), written as NATURAL SENTENCES — not a comma keyword list>", "duration":5, "aspect_ratio":"<ratio>" }
    • stock (generic real-world beat): { "op":"search", "kind":"image|video", "query":"3-5 keywords", "aspect_ratio":"<ratio>", "source":"pexels", "count":1 } — ALWAYS include aspect_ratio. SOURCE: default "pexels"; use "archive" (Internet Archive — historical/vintage/archival/higher-quality) when the user asks for archival/old/documentary/better footage, or "openverse"/"wikimedia" for free/CC. Match the user's request.
- "DIALOGUE [Name]: …" line → a TALKING shot: that named character speaks on camera. Output: { "op":"generate", "kind":"video", "talk":true, "line":"<the EXACT dialogue words>", "prompt":"<the character's fixed look + the setting/mood + WHATEVER camera fits the beat — ANY angle, close-up, push-in, pan, movement: full creative freedom>", "aspect_ratio":"<ratio>" }. WRITE THE PROMPT AS NATURAL SENTENCES (plain full sentences, e.g. "Maya is a young woman with long red hair in a yellow raincoat. She stands in the rain and turns toward the camera.") — do NOT use a comma-separated keyword/tag list; tag-piles BREAK the video model's lip-sync. Frame the camera FREELY (that freedom is the whole point of this shot type) — the editor appends the spoken line itself in the required format. Put the SPOKEN WORDS ONLY in "line" — do NOT put them in the prompt. Put the aspect ratio ONLY in the "aspect_ratio" field — NEVER write "16:9"/"9:16" (or a duration) inside the prompt text. ALWAYS include "line" and do NOT set "duration". "talk":true marks it a lip-sync shot. (EXCEPTION — ONLY if a REFERENCE IMAGE is attached, the shot is audio-lip-synced, so THEN keep the character clearly FACING the camera; with no reference the camera is fully free.)
...
REFERENCE IMAGE: if the user message says a REFERENCE IMAGE is attached, the character(s) come FROM it — do NOT invent or describe their look. For image shots write a SHORT Flux-EDIT prompt = ONLY the changes (wardrobe/setting/pose), MOST IMPORTANT first, ending "keep the same face and identity, do not change anything else". Talking (talk:true) shots keep the SAME rule — describe only the setting/pose (a ref shot IS audio-lip-synced, so keep the character facing the camera); the spoken words stay in "line", never in the prompt.
